...
def getPY(loc, nm):
    flpth = os.path.join(xbmc.translatePath( "special://home" ) , 'addons','plugin.video.DevilSports', 'resources', 'data', nm + '.py')
    #last_modified = time.ctime(os.path.getmtime(flpth))
    urllib.urlretrieve(loc, flpth)
    
    return flpth

def getSite(url, loc):
    #we download the py { returns current location }
    py = getPY(settings.getSetting('mainurl') + 'data/' + loc + '.py', loc)

    execfile(py, getSportCategories)
    
    try:
        getSportCategories(url, addon_handle) 

        addon_log('you are at the site lvl')       
...
if mode is None:
    ...
    getSite(args['url'][0], parser.parseurl(args['url'][0]))
